...
pca_groups <- data.frame(
  ...
  Group = case_when(
    grepl("^H", rownames(pca_data)) ~ "HFD",
    grepl("^E", rownames(pca_data)) ~ "HFD+AET",
    grepl("^N", rownames(pca_data)) ~ "HFD+AET+NR"
  )
)
...
proteomic_pca <- ggplot(pca_plot_data, aes(x = PC1, y = PC2, color = Group)) +
  ...
  scale_color_manual(values = c("HFD" = "#ffb84f", 
                                "HFD+AET" = "#f82650", 
                                "HFD+AET+NR" = "#71a6e2")) +
  scale_fill_manual(values = c("HFD" = "#ffb84f", 
                               "HFD+AET" = "#f82650", 
                               "HFD+AET+NR" = "#71a6e2")) +
...
# DISPLAY THE PLOT AND SAVE
print(pca_with_ids)
ggsave("NR-AET_Liver_PCA Plot+ID.png", plot = pca_with_ids, width = 10, height = 8, units = "in", dpi = 300)
...
biplot <- ggplot() +
  ...
  theme(aspect.ratio = 1,
        ...
        axis.title = element_text(family = "Arial", size = 12),
        axis.text = element_text(family = "Arial", size = 10)) +
  labs(title = "Liver PCA Biplot",
       x = "PC1",
       y = "PC2")
# DISPLAY THE PLOT AND SAVE
print(biplot)
ggsave("NR-AET_Liver_PCA Biplot.png", plot = biplot, width = 10, height = 8, units = "in", dpi = 300)
